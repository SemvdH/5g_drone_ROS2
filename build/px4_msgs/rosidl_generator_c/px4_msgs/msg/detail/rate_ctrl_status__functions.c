  ...
  px4_msgs__msg__RateCtrlStatus__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    px4_msgs__msg__RateCtrlStatus * data =
      (px4_msgs__msg__RateCtrlStatus *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!px4_msgs__msg__RateCtrlStatus__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          px4_msgs__msg__RateCtrlStatus__fini(&data[i]);
        }
        free(data);
        return false;
      }
