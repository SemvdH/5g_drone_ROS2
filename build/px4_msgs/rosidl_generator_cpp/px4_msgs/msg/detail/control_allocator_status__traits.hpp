  ...
  std::ostream & out)
{
  ...
  {
    if (msg.allocated_torque.size() == 0) {
      out << "allocated_torque: []";
    } else {
      out << "allocated_torque: [";
      size_t pending_items = msg.allocated_torque.size();
      for (auto item : msg.allocated_torque) {
  ...
  {
    if (msg.allocated_thrust.size() == 0) {
      out << "allocated_thrust: []";
    } else {
      out << "allocated_thrust: [";
      size_t pending_items = msg.allocated_thrust.size();
      for (auto item : msg.allocated_thrust) {
        rosidl_generator_traits::value_to_yaml(item, out);
        if (--pending_items > 0) {
          out << ", ";
        }
