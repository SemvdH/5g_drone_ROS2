...
bool px4_msgs__msg__geofence_result__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // home_required
    PyObject * field = PyObject_GetAttrString(_pymsg, "home_required");
    if (!field) {
      return false;
    }
    assert(PyBool_Check(field));
    ros_message->home_required = (Py_True == field);
    Py_DECREF(field);
  }
...
PyObject * px4_msgs__msg__geofence_result__convert_to_py(void * raw_ros_message)
{
  ...
  {  // home_required
    PyObject * field = NULL;
    field = PyBool_FromLong(ros_message->home_required ? 1 : 0);
    {
      int rc = PyObject_SetAttrString(_pymessage, "home_required", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
